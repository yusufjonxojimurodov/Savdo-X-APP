...
<template>
    ...
    <div class="container flex justify-between items-center flex-wrap !mb-[20px]">
        <template v-if="productStore.loader && segmentValue === 'all'">
        ...
        <template v-else-if="segmentValue === 'all'">
            <ion-card v-for="product in productStore.products" :key="product._id" @click="setProductId(product._id)">
        ...
        <a-empty v-if="!productStore.loader && productStore.products.length === 0 && segmentValue === 'all'"
            class="!text-white relative top-40 !mb-50 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <template #image>
