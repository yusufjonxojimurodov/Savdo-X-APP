...
import WatchesFilterComponent from './WatchesFilterComponent.vue';
...
<template>
    ...
    <div class="container !p-4">
        <div class="flex justify-between items-center w-full">
            <ion-back-button @click="router.push('/')" mode="ios" defaultHref="/" type="dark"
                text="Orqaga"></ion-back-button>

            <h1 class="title-page text-[#1e293b] !font-semibold !m-0">Soatlar</h1>
        </div>
    </div>
    <banner-component />
    <watches-filter-component />
    <div class="container flex justify-between items-center flex-wrap !mb-[20px]">
        <ion-card @click="setProductId(product._id)" v-for="product in filterProductStore.watches" :key="product._id">
        ...
        <a-empty class="!text-white relative top-40 !mb-50 left-1/2 -translate-x-1/2 -translate-y-1/2"
            v-if="!filterProductStore.loader && filterProductStore.watches.length === 0">
            <template #image>
